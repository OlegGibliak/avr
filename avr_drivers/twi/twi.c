...
#define TWI_START_CONDITION() (TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWSTA))
#define TWI_RESTART_CONDITION() TWI_START_CONDITION
#define TWI_STOP_CONDITION()  (TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWSTO))
...
/*                         Static global vars                                */
/*****************************************************************************/
static twi_descriptor_t m_desc;
/*****************************************************************************/
/*                         Static function                                   */
/*****************************************************************************/
...
ISR(TWI_vect)
{
    switch (TWSR)
    {
        ...
        {
            TWCR = (1 << TWINT) | (1 << TWEN) | ((!m_desc.len == 1) ? (0 << TWEA) : (1 << TWEA));
            break;
        }
...
void twi_init(twi_scl_t f_scl, twi_tx_callback tx_cb, twi_rx_callback rx_cb)
{
    ...
    TWCR = (1 << TWEN) |                                        /* < Enable module           */
           ...
           (0 << TWAE);                                         /* < Disable acknowledge Bit */
}

error_t twi_send(uint8_t addr, const void* p_data, uint8_t len)
{
...
error_t twi_read(uint8_t addr, void* p_data, uint8_t len)
{
    if (TWI_STATE_IDLE != m_desc.state)
    {
        return ERROR_BUSY;
